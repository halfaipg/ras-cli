...
export class Logger {
  private rasDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.rasDir = getProjectTempDir(process.cwd());
this.logFilePath = path.join(this.rasDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.rasDir, { recursive: true });
  ...
  _checkpointPath(tag: string): string {
    ...
    return path.join(this.rasDir, `checkpoint-${sanitizedTag}.json`);
  }
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.rasDir) {
      console.error(
